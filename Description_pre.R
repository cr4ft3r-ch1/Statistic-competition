...
summary(panel_data_pre$pre_education_expenses_perexpenditure)

ggplot2::ggplot(panel_data_pre) +
  ggplot2::geom_histogram(
    aes(x = pre_education_expenses_perexpenditure),
    bins = 20
  ) +
  ggplot2::facet_wrap( ~ metro_area) + 
  ggplot2::labs(title = "都市区分による教育費割合の分布(県単位)",
                x = "教育費割合",
                y = "県数") +
  ggplot2::facet_wrap( ~ metro_area) +
...
ggplot2::ggplot(panel_data_pre) +
  ggplot2::geom_histogram(
    aes(x = pre_education_expenses_perstudents),
    bins = 20
  ) + 
  ggplot2::labs(title = "年別の生徒一人当たり教育費の分布(県単位)",
                x = "生徒一人当たり教育費",
                y = "県数") +
...
ggplot2::ggplot(panel_data_pre) +
  ggplot2::geom_histogram(
    aes(x = log(pre_education_expenses_perstudents)),
    bins = 20
  ) +
  ggplot2::labs(title = "年別の対数を取った生徒一人当たり教育費の分布(県単位)") +
  ggplot2::facet_wrap( ~ new_year)

# 地方税
ggplot2::ggplot(panel_data_pre) +
  ggplot2::geom_histogram(
    aes(x = pre_local_tax_perpop),
    bins = 20
  )

ggplot2::ggplot(panel_data_pre) +
  ggplot2::geom_histogram(
    aes(x = log(pre_local_tax_perpop)),
    bins = 20
  ) +
  ggplot2::labs(title = "年別の対数を取った一人当たり地方税年別分布(県単位)") +
  ggplot2::facet_wrap( ~ new_year)
...
# =========================================================
# メリット: 下三角に散布図、対角線に分布、上三角に相関係数が表示され、ggplot2ベースで美しい。
